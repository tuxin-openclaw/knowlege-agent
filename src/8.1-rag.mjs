...
const questions = ["光光是怎样帮助东东学会踢足球的？"];
...
for (const question of questions) {
  ...
  // 获取相似度评分
  const scoredRess = await vectorStore.similaritySearchWithScore(question, 3);

  console.log("\n【检索到文档及相似度评分");
  retrievedDocs.forEach((doc, index) => {
    const scoredRes = scoredRess.find(
    ...
    console.log(
      `元数据：章节=${doc.metadata.chapter}, 角色=${doc.metadata.character}, 类型=${doc.metadata.type}, 情绪=${doc.metadata.mood}`,
    );
  });
